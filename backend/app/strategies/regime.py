...
def market_regime(
    ...
    if above_long:
        score += 0.4
        reasons.append(f"benchmark above {long_win}d SMA")
    else:
        score -= 0.4
        reasons.append(f"benchmark below {long_win}d SMA")

    # 2) Golden / death cross of the 50d vs the long SMA.
    if sma_fast > sma_slow:
        score += 0.2
        reasons.append("50d above long-term SMA (golden)")
    else:
        score -= 0.2
        reasons.append("50d below long-term SMA (death)")
    ...
    score += clamp(slope * 20.0, -0.2, 0.2)  # ~1%/20d move ≈ full tilt
    ...
    if dd <= -0.10:
        score -= 0.2
        reasons.append(f"{dd:.0%} off the {hi_win}d high")

    # 5) Optional breadth confirmation.
    if breadth_pct is not None:
        score += clamp((breadth_pct - 0.5) * 0.8, -0.2, 0.2)
        reasons.append(f"breadth {breadth_pct:.0%} above trend")
